...
import { ArrowLeft, Phone, Mail, MapPin, Globe, ExternalLink, Building2 } from "lucide-react";
    ...
    <div className="space-y-6">
        ...
        <div className="space-y-1">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {prospect.businessType && (
              <span className="text-sm text-muted-foreground">{prospect.businessType}</span>
            )}
          </div>
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="space-y-6 lg:col-span-1">
          <div className="rounded-xl border bg-card p-5 shadow-sm">
            ...
            <dl className="mt-4 space-y-3 text-sm">
              ...
              <div className="flex items-center justify-between">
                ...
                <dd>{prospect.email ? <a href={`mailto:${prospect.email}`} className="hover:underline">{prospect.email}</a> : "—"}</dd>
              </div>
              <div className="flex items-center justify-between">
                <dt className="flex items-center gap-2 text-muted-foreground">
                  <MapPin className="h-4 w-4" /> Adresse
                </dt>
                <dd>{[prospect.city, prospect.department, prospect.country].filter(Boolean).join(", ") || "—"}</dd>
              </div>
              <div className="flex items-center justify-between">
                <dt className="flex items-center gap-2 text-muted-foreground">
                ...
                <dd>
                  {prospect.website ? (
                    <a href={prospect.website} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 hover:underline">
              ...
              <div className="flex items-center justify-between">
                ...
                <dd>
                  {prospect.googleBusinessUrl ? (
                    <a href={prospect.googleBusinessUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 hover:underline">
                      Voir <ExternalLink className="h-3 w-3" />
                    </a>
                  ) : (
                    "—"
              ...
              {prospect.googleRating && (
                <div className="flex items-center justify-between">
                  ...
                  <dd>{prospect.googleRating} / 5 ({prospect.googleReviewCount ?? 0} avis)</dd>
                </div>
              )}
            </dl>
          </div>
          ...
          <div className="rounded-xl border bg-card p-5 shadow-sm">
            <h2 className="text-sm font-semibold uppercase tracking-wide text-muted-foreground">Valeur</h2>
            <dl className="mt-4 space-y-3 text-sm">
              <div className="flex items-center justify-between">
                <dt className="text-muted-foreground">Valeur potentielle</dt>
                <dd>{prospect.estimatedValue ? `${prospect.estimatedValue.toLocaleString("fr-FR")} €` : "—"}</dd>
              </div>
              <div className="flex items-center justify-between">
                <dt className="text-muted-foreground">Mise en service</dt>
                <dd>{prospect.setupFee ? `${prospect.setupFee.toLocaleString("fr-FR")} €` : "—"}</dd>
              </div>
              <div className="flex items-center justify-between">
                <dt className="text-muted-foreground">Prix mensuel</dt>
                <dd>{prospect.monthlyPrice ? `${prospect.monthlyPrice.toLocaleString("fr-FR")} €/mois` : "—"}</dd>
              </div>
              <div className="flex items-center justify-between">
                <dt className="text-muted-foreground">Achat définitif</dt>
                <dd>{prospect.oneTimePrice ? `${prospect.oneTimePrice.toLocaleString("fr-FR")} €` : "—"}</dd>
              </div>
            </dl>
          </div>
        </div>
