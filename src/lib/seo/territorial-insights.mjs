...
export function localInsights(axis,c,e){
 ...
 const missingPostal=!c.postalCodes.length;
 const postalCoverageKnown=!missingPostal&&c.postalCodes.every(code=>e.postalGroups.some(p=>p.postalCode===code&&p.totalCommunes>=1));
 ...
 const local=[{
  ...
  fact:missingPostal?`Aucun code postal n’est renseigné pour ${c.name} dans cette extraction. Cette absence ne prouve pas que la commune n’en possède pas ; le lieu reste à confirmer avec l’adresse.`:shared.length?`Le code postal ne suffit pas ici : ${postal}. ${shared[0].others.length?`Par exemple, ${shared[0].others.map(n=>n.name+' ('+n.code+')').join(', ')} partage${shared[0].others.length>1?'nt':''} le code ${shared[0].postalCode}.`:''}`:postalCoverageKnown?`Dans cette extraction, ${c.name} est la seule commune associée à ${postal}. Cette observation doit être réévaluée si le référentiel évolue.`:`Les codes postaux fournis pour ${c.name} sont ${postal}, mais leur rapprochement avec les autres communes est incomplet. Il faut vérifier l’adresse et la commune sans déduire l’absence de partage.`,
...
export function localQuestions(axis,c,e){return [
 {q:`Le code postal suffit-il pour une demande à ${c.name} ?`,a:!c.postalCodes.length?`Aucun code postal n’est renseigné dans cette extraction pour ${c.name}. Demandez au contact de préciser la commune et l’adresse ; ne reconstituez pas le code postal. Le code INSEE ${c.code} identifie la commune, pas une adresse postale.`:e.postalGroups.some(p=>p.totalCommunes>1)?`Non : au moins un code postal de ${c.name} est partagé par plusieurs communes dans la source. Demandez aussi la commune et conservez son code INSEE ${c.code}.`:!c.postalCodes.every(code=>e.postalGroups.some(p=>p.postalCode===code&&p.totalCommunes>=1))?`Le rapprochement des codes postaux est incomplet dans cette extraction. Ne concluez pas à un code exclusif : demandez la commune et l’adresse, puis conservez le code INSEE ${c.code} séparément.`:`La source n'indique pas de partage pour les codes postaux de ${c.name}. Il reste utile de demander la commune et l'adresse ; le code ${c.code} identifie la commune, pas le client.`},
