...
let links=0;
for(const entry of entries){const route=draftPath(entry.axis,entry.commune),html=readFileSync(`dist${route}index.html`,'utf8');assert.match(html,/<meta[^>]+name="robots"[^>]+content="[^"]*noindex/);assert(html.includes('Brouillon enrichi'));assert(html.includes('Ce que les données'));assert(html.includes('territory-planner'));assert(html.includes('project-guides'));assert(html.includes('implementation-dossier'));assert(html.includes('territorial-brief'));assert(html.includes('Télécharger mon brouillon'));assert(html.includes('/preparation/departements/'));assert(html.includes(`territory:${entry.axis}:${entry.commune.code}`));assert(html.includes('Décisions à prendre avant publication'));assert(html.includes('https://geo.api.gouv.fr/communes/'+entry.commune.code));for(const match of html.matchAll(/href="(\/[^"?#]*)/g)){const href=match[1];if(!href.endsWith('/'))continue;assert(existsSync(`dist${href}index.html`),`Missing ${href} on ${route}`);links++;}}
